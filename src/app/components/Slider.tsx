...
const SalePage = () => {
  return (
    <Carousel
      ...
      className="w-full overflow-x-hidden flex mt-3 "
    >
      <CarouselContent>
        {Slider.map((item, index) => (
          <CarouselItem key={index} className="flex justify-center px-4">
          <div className="flex flex-col md:flex-row items-center mx-auto bg-white rounded-lg shadow-lg overflow-hidden w-full max-w-5xl">
            ...
            <div className="relative w-full md:w-1/2 min-h-[12rem] md:min-h-[15rem] flex justify-center items-center">
              <Image
                src={item.url}
                alt={`Slider ${item.id}`}
                fill
